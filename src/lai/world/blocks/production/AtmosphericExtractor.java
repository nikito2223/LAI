...
public class AtmosphericExtractor extends GenericCrafter {
    ...
    public void init(){
        ...
        // Заглушка по умолчанию (если блок используется вне планеты)
        if(plans.isEmpty()){
            plans.add(new LiquidPlan(Liquids.hydrogen, 5f, 60f));
        }
    }
    ...
    public class AtmosphericExtractorBuild extends GenericCrafterBuild {
        ...
        public Seq<LiquidPlan> plans = new Seq<>();
        public LiquidPlan selectedPlan;
        public float progress = 0f;
        public boolean loadedAtmosphere = false;
    
        @Override
        public void updateTile() {
    
            // 1) Загружаем атмосферу только один раз
            if(!loadedAtmosphere){
                loadedAtmosphere = true;
    
                plans.clear(); // Сбрасываем старые газы!
    
                Planet current = (Vars.state == null || Vars.state.rules == null || Vars.state.rules.sector == null)
                    ? null
                    : Vars.state.rules.sector.planet;
    
                if(current instanceof LaiPlanet lai){
                    Seq<LiquidStack> gases = lai.getAtmosphereGases();
    
                    if(gases != null && gases.size > 0){
                        for(LiquidStack st : gases){
                            plans.add(new LiquidPlan(st.liquid, 10f, 60f));
                        }
                    }
                }
    
                // если планета не дала газов → добавляем только заглушку
                if(plans.isEmpty()){
                    plans.add(new LiquidPlan(Liquids.hydrogen, 10f, 60f));
                }
        ...
        public Object config() {
            return selectedPlan == null ? 0 : plans.indexOf(selectedPlan);
        }
        ...
        public void write(Writes write) {
            super.write(write);
            write.i(plans.indexOf(selectedPlan));
        }
    }
