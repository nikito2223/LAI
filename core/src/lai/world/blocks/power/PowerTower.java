...
public class PowerTower extends PowerBlock {
    ...
    public void load(){
		...
		laserEnd = Core.atlas.find(name + "-end");
    }
    ...
    public static void getNodeLinks(Tile tile, Block block, Team team, Cons<Building> others){
        ...
        for(int i = 0; i < 4; i++){
            ...
            int fi = i;
            Building closest = tempBuilds.min(b -> b instanceof PowerTowerBuild node && node.couldConnect((fi + 2) % 4, block, tile.x, tile.y), b -> b.dst2(cx, cy));
            tempBuilds.clear();
            if(closest != null){
                others.get(closest);
            }
    ...
    public class PowerTowerBuild extends Building{
        ...
        public void draw(){
            ...
            if(Mathf.zero(Renderer.laserOpacity) || team == Team.derelict) return;
       		Color blendedColor = laserColor1.cpy().lerp(team.color, 0.6f);

            Draw.z(Layer.power);
            Draw.color(blendedColor, team.color, (1f - power.graph.getSatisfaction()) * 0.86f + Mathf.absin(3f, 0.1f));
            Draw.alpha(Renderer.laserOpacity);
            float w = laserWidth + Mathf.absin(pulseScl, pulseMag);

            for(int i = 0; i < 4; i ++){
                if(dests[i] != null && links[i].wasVisible && (!(links[i].block instanceof PowerTower node) ||
