...
public class ObjParser {
    ...
    public static Obj loadObj(String s) {
        ...
        try {
            Obj o = loadObj(
                    LaiVars.internalFileTree.child("models/" + s + ".obj").readString(),
                    LaiVars.internalFileTree.child("models/" + s + ".mtl").readString()
            );
            loaded.put(s, o);
            return o;
        } catch (Exception e) {
                ...
                try {
                    switch (parts[0]) {
                        ...
                        case "illum":
                            ...
                            break;
                        case "map_Kd":
                            Texture t = new Texture(LaiVars.internalFileTree.child("models/" + parts[1]));
    ...
    private static Obj loadObj(String obj, String mtl) throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new StringReader(obj))) {
            ...
            flushMesh.run();
            LaiLog.load("loaded " + result.materials.size + " parts of 3d model, " + v + " vertices, " + i + " indices");
            return result;
        }
    }
